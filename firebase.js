...
export async function sendMessage(channelId, senderUid, text, type = "text", meta = {}) {
  ...
  await addDoc(messagesRef, { senderUid, text, type, meta, sentAt: serverTimestamp() });
...
export async function sendInvoicePdf(channelId, senderUid, pdfData, invoiceId) {
  ...
  await sendMessage(channelId, senderUid, `📄 Számla: ${invoiceId}`, "invoice", { invoiceId, pdfUrl: url });
  return url;
}
